...
| -------- | ---------------------------------------------------------------------------------- |
| 项目信息 | 描述您的应用信息                                                                   |
| 插件配置 | 描述您的应用依赖哪些 CloudBase Framework 插件，根据配置来构建和部署您的应用        |
| 生命周期 | 配置在构建部署生命周期前后，需要执行的自定义动作                                   |
| 应用依赖 | 在云端一键部署场景下，您需要完善应用依赖配置来声明应用依赖的扩展资源和环境变量参数 |
| 模板变量 | 您可以在配置当中使用模板变量来实现动态配置                                         |
